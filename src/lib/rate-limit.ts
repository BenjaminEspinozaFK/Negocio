 ...
 */
export async function getRemainingBlockTime(ipAddress: string): Promise<number> {
  const windowStart = new Date(Date.now() - RATE_LIMIT_CONFIG.windowMinutes * 60 * 1000);
  ...
  const blockUntil = new Date(
    lastFailedAttempt.timestamp.getTime() + RATE_LIMIT_CONFIG.blockDurationMinutes * 60 * 1000
  );
